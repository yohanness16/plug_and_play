...
import { blogPosts, blogPostCategories, users, } from '../db/schema.js';
...
async function makeUniqueSlug(baseSlug, excludeId) {
    ...
    while (true) {
        const existing = await db
            .select()
            .from(blogPosts)
            .where(and(eq(blogPosts.slug, unique), excludeId ? sql `(${blogPosts.id} <> ${excludeId})` : undefined));
...
posts.get('/', async (c) => {
    ...
    if (q.status)
        filters.push(eq(blogPosts.status, q.status));
    if (q.authorId)
        filters.push(eq(blogPosts.authorId, q.authorId));
    if (q.categoryId) {
...
posts.put('/:id', requireAuth, async (c) => {
    ...
    if (data.title && !data.slug)
        finalSlug = slugify(data.title).toLowerCase();
    ...
    await db.transaction(async (tx) => {
        ...
        if (data.categories) {
            ...
            if (data.categories.length) {
                const mappings = data.categories.map((categoryId) => ({ postId, categoryId }));
...
posts.delete('/:id', requireAuth, async (c) => {
    ...
    if (hard) {
        if (user.role !== 'admin')
            return c.json({ error: 'Admin only for hard delete' }, 403);
