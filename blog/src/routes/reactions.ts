...
async function getCommentCounts(commentId: string) {
...
const postId = c.req.param('postId');
  if (!postId) return c.json({ error: 'Missing postId param' }, 400)
  ...
  if (!post || post.length === 0) return c.json({ error: 'Post not found' }, 404)
...
reactions.get('/posts/:postId/reactions', async (c) => {
 ...
 const postId = c.req.param('postId');
  ...
  if (user) {
    ...
    if (existing && existing.length) userReaction = existing[0].type
  }
...
reactions.post('/comments/:commentId/reactions', requireAuth, async (c) => {
  ...
  if (!commentId) return c.json({ error: 'Missing commentId param' }, 400);
  ...
  if (!comment || comment.length === 0) return c.json({ error: 'Comment not found' }, 404);
